...
for c_label in np.unique(bubble_label_image):  # one bubble at a time
    if c_label > 0:  # ignore background
        ...
        out_results += [
            {"x": xx[cur_roi].mean(), "y": yy[cur_roi].mean(), "z": zz[cur_roi].mean(), "volume": np.sum(cur_roi),}
        ]
...
train_values = pd.read_csv(args.training_bubble_volume_stats_filename)  # "../input/bubble_volume.csv"
...
ax2.plot(out_table["x"], out_table["y"], "r.", train_values["x"], train_values["y"], "b.")
ax2.legend(["Watershed Bubbles", "Training Bubbles"])

plt.save(args.bubble_stats_comparison_plot_output_filename)
