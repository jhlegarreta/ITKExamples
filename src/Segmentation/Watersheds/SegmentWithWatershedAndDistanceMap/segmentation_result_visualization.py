...
plt.save(args.pipeline_images_output_filename)
...
plt.save(args.segmentation_projections_output_filename)
...
fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(12, 4))
for i, (cax, clabel) in enumerate(zip([ax1, ax2, ax3], ["xy", "zy", "zx"])):
    cax.imshow(np.max(bubble_label_image, i), cmap=plt.cm.jet, vmin=0, vmax=new_idx)
    cax.set_title("{} Projection".format(clabel))
    cax.set_xlabel(clabel[0])
    cax.set_ylabel(clabel[1])

plt.save(args.clean_segmentation_projections_output_filename)
...
def show_3d_mesh(image, thresholds):
    ...
    for _i, c_threshold in tqdm(list(enumerate(thresholds))):
        verts, faces, _, _ = measure.marching_cubes(p, level=c_threshold, 0)
...
fig.save(args.volume_rendering_output_filename)
